...
class SubdivxSubtitle(Subtitle):
    ...
    def guess_spanish_neutral(self):
        TRUSTED_LATINO_UPLOADERS = ['TaMaBin', 'oraldo',
                                    'enanodog', 'antillan0', 'gozilla2', 'axel7902']
...
class SubdivxProvider(Provider):
        ...
        while True:
            ...
            for counter, match in enumerate(subtitle_re.finditer(str(soup))):
                ...
                subtitle = self.subtitle_class(
                    language, subtitle_url, subtitle_id, video_name, description, downloads, uploader)
    ...
    def guess_language(self, description, uploader):
        ...
        NON_SPANISH_REFERENCES = ['ingles', 'inglés',
                                  'francés', 'frances', 'portugues', 'portugués']
        LATINO_REFERENCES = ['neutro', 'Neutro', 'NEUTRO', 'latino', 'Latino', 'LATINO', 'Latinoamérica',
                             'latinoamérica', 'latinizado', 'latina', 'Latina', 'neutral']
    ...
    def list_subtitles(self, video, languages):
        ...
        if isinstance(video, Episode):
            if self.series_video_name_has_year(video.name):
                query = video.series+"." + \
                    str(video.year)+".S"+str(video.season).zfill(2) + \
                    "E"+str(video.episode).zfill(2)
            else:
                query = video.series+".S" + \
                    str(video.season).zfill(2)+"E"+str(video.episode).zfill(2)
